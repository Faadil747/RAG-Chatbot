...
4. **Environment variables**:
   ...
   `OMP_NUM_THREADS=1` caps PyTorch's CPU thread pool — on a free-tier instance with a fraction of a CPU core, the default (one thread per detected core) buys nothing and its per-thread stack overhead is pure waste.

   This service is tuned for a 512MB container in several ways, layered on top of each other because no single one was enough on its own:
   - `requirements.txt` pins PyTorch to its CPU-only wheel (the default PyPI wheel is the much larger CUDA build, which this service never uses).
   - `EMBEDDING_MODEL` is `BAAI/bge-small-en-v1.5` (~130MB of weights) rather than a larger model.
   - The PDF/DOCX parsing libraries (PyMuPDF, pdfplumber, python-docx) and the `groq` SDK are imported lazily, inside the functions that actually use them, instead of at module load time — a process that's only ever served health checks and searches never pays to have them resident in memory.
   - The embedding model itself is also loaded lazily, on first use, rather than blocking startup (see `main.py`'s `lifespan`) — this means `uvicorn` binds its port almost immediately, so the platform's boot-time port scan succeeds and the health check goes green right away; the actual model/torch import cost is deferred to the first request that needs it (parse, index, search, chat, or an analytics query with a skill filter).
   
   If it still runs out of memory after all of this as your candidate pool or traffic grows, that's a real signal to upgrade this service's Render plan rather than trying to squeeze further — fight the platform's resource limits with resources, not more code.
5. **Health check path**: `/ai/health`.
6. Deploy. Because model loading is now deferred past startup, the health check should go green quickly. The *first* real request that needs the embedding model (a search, an upload, or a chat "find..." query) will be noticeably slower than the rest — that's the one-time model load happening, not a hang. Watch the logs for `"AI service ready. N candidates indexed."` on boot, and for a "Loading embedding model..." line on that first real request.
